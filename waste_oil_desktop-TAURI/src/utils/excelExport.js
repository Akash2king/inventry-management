...
/**
 * Build an .xlsx and save it. In Tauri, opens a Save dialog (SheetJS writeFile does not work reliably in webviews).
 * In the browser / Electron renderer, triggers a file download via Blob.
 *
...
export async function downloadExcelFile(filename, sheetName, headers, rows) {
  ...
  if (isTauriRuntime()) {
    try {
      ...
      console.warn("[excelExport] Tauri save/write failed, falling back to download", e);
