...
export function ExecutiveDashboard() {
  ...
  const overdueCount = filteredRecords.filter((r) => (r.alert_level || "").toLowerCase() === "red").length;
  ...
  }, [filteredRecords, lookbackDays, today, showHeavySections]);

  const trendMax = Math.max(1, ...trendData.map((d) => d.count));

  const unitOptions = useMemo(() => {
    if (!showHeavySections) return ["all", "Kgs"];
    const units = new Set();
    filteredRecords.forEach((r) => {
  ...
  }, [filteredRecords, showHeavySections]);
  ...
  const topVendors = useMemo(() => {
    ...
    return Array.from(map.values())
      ...
      .slice(0, 5);
  }, [filteredRecords, vendorUnitFilter, showHeavySections]);
  ...
  const agingBuckets = useMemo(() => {
    if (!showHeavySections) {
      return { "0-7": 0, "8-15": 0, "16-30": 0, "30+": 0 };
    }
    const buckets = {
      "0-7": 0,
      "8-15": 0,
    ...
    return buckets;
  }, [filteredRecords, today, showHeavySections]);
  ...
  const toggleAdvancedAnalytics = () => {
    setShowHeavySections((prev) => {
      const next = !prev;
      if (next) {
        showToast("Advanced analytics turned on.", "success");
      }
      return next;
    });
  };
    ...
    <div className="dashboard-page">
      ...
      <div className="card dashboard-toolbar dashboard-toolbar-simple" style={{ marginBottom: "1rem", padding: "0.85rem 1rem" }}>
        <div className="dashboard-toolbar-row" style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", alignItems: "center" }}>
          ...
          <button
            type="button"
            className="btn btn-ghost dashboard-filter-chip"
      ...
      <div className="dashboard-advanced-toggle">
        <button
          type="button"
          className="btn btn-ghost"
          onClick={toggleAdvancedAnalytics}
          aria-pressed={showHeavySections}
        >
          {showHeavySections ? "Hide advanced analytics" : "Show advanced analytics"}
        </button>
        <span className={`dashboard-advanced-status ${showHeavySections ? "is-on" : ""}`}>
          {showHeavySections ? "ON" : "OFF"}
        </span>
      </div>
      ...
      <div className="dashboard-kpi-grid">
        ...
        <KPICard
          variant="overdue"
          label="Overdue"
          hint="Records currently in red alert."
          value={overdueCount}
          subtext="Matches red badges in the table."
      ...
      <div className="dashboard-mobile-panels">
        <div className="card">
          ...
          {inProgressInScope.length === 0 ? (
            <div className="dashboard-table-empty">No in-progress records in this scope.</div>
          ) : (
            <div className="dashboard-mobile-list">
              {inProgressInScope.slice(0, 10).map((r) => (
                <button key={r.id} type="button" className="dashboard-mobile-record" onClick={() => navigate(`/records/${r.id}`)}>
                  ...
                  <div className="dashboard-mobile-record__meta">
                    {r.vendor_name || "Unknown Vendor"} - {STAGE_LABELS[Number(r.current_stage)] || r.current_stage}
                  </div>
                  <div className="dashboard-mobile-record__meta">
        ...
        <div className="card">
          ...
          {completedInScopeSorted.length === 0 ? (
            <div className="dashboard-table-empty">No completed records in this scope.</div>
          ) : (
            <div className="dashboard-mobile-list">
              {completedInScopeSorted.slice(0, 10).map((r) => (
                <button key={r.id} type="button" className="dashboard-mobile-record" onClick={() => navigate(`/records/${r.id}`)}>
                  <div className="dashboard-mobile-record__head">
                    <strong>{r.record_number}</strong>
                    <StatusBadge level={r.alert_level} />
      ...
      <div className="card" style={{ marginBottom: "1rem" }}>
        ...
        {dueSoonRecords.length > 0 ? (
          <div style={{ marginTop: "0.35rem" }}>
            <button type="button" className="btn btn-ghost btn-sm" onClick={exportDueSoonExcel}>
              Export this list
            </button>
          </div>
        ) : null}
      ...
      {showHeavySections ? (
        <>
      <div className="dashboard-layout-grid--primary">
        <div className="card">
      ...
      <div className="dashboard-layout-grid--secondary">
        {!analyticsError && (
      ...
      <div className="dashboard-layout-grid--tertiary">
        <div className="card">
        ...
        <div className="card" style={{ marginTop: "1rem", marginBottom: "1rem", opacity: 0.85 }}>
          Advanced analytics are hidden to keep this screen focused. Open them when you need deeper trends.
        </div>
      )}
