  ...
  return (
    <div className="page-records">
      ...
      <div className="mobile-list">
        ...
        {!isLoading && records.length === 0 ? (
          <div className="card mobile-list__state">No records</div>
        ) : null}
        {!isLoading &&
          records.map((r) => (
            <button
              key={r.id}
              type="button"
              className="card mobile-record-card"
              onClick={() => navigate(`/records/${r.id}`)}
            >
              ...
              <div className="mobile-record-card__meta">{r.vendor_name || "—"}</div>
              <div className="mobile-record-card__meta">
                Stage {r.current_stage} • {formatQty(r.quantity, r.unit)}
              </div>
              <div className="mobile-record-card__meta">
                Entry {formatDate(r.entry_date)} • Due {formatDate(r.due_date)}
              </div>
              <div className="mobile-record-card__meta">{formatHolderLine(r)}</div>
            </button>
          ))}
      </div>
