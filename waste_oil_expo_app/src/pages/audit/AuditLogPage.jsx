        ...
        groups.map((g) => (
          <div className="card" key={g.group} style={{ marginBottom: "1rem" }}>
            ...
            <div className="mobile-list">
              {g.items.map((r) => (
                <div key={r.id} className="card mobile-audit-card">
                  <div className="mobile-audit-card__head">
                    <strong>{r.action || "—"}</strong>
                    <span>{formatTs(r.timestamp)}</span>
                  </div>
                  <div className="mobile-audit-card__meta">
                    {r.username ? `@${r.username}` : "System"} • Record {r.record_number || "—"}
                  </div>
                  <div className="mobile-audit-card__desc">{r.description || "—"}</div>
                </div>
              ))}
            </div>
          </div>
